...
import com.spark.member.repository.MemberRepository;
...
public class MemberServiceImpl implements MemberService {
  ...
  public LoginResult login(MemberDto m) {
    ...
    // 1. 유저 확인
    MemberDto memberDto = memberRepository.login(m)
            .orElseThrow(() -> new CustomException("회원정보가 없습니다.", 400));

    memberValidator.validationPassword(m.getMemPwd(), memberDto.getMemPwd());
    
    // 심현섭 메세지 : 여기 아래 부분도 적절히 클래스 분리해서 처리해 볼 것.
    // 2. 토큰 생성
    ...
    int result = memberRepository.insertRefreshToken(map); // db에 refreshtoken 저장
    if (result == 0) {
  ...
  @Override
  public ValidateResponse loginUserInfo(String authHeader) {

    String token = authHeader.replace("Bearer ", ""); // Bearer를 제거해 token만 추출
    memberValidator.validationToken(token); // 토큰 유효성 검사
    
    String memberId = jwtProvider.getUserId(token);
    MemberDto memberDto = memberRepository.loginUserInfo(memberId);

    return ValidateResponse.available(memberDto);
  }

  @Override
  public MemberDto findById(String userId) {

    MemberDto result = memberRepository.findById(userId);
    if (result == null) {
  ...
  public LogoutResult deleteToken(String refreshTokenHeader) {
    ...
    int result = memberRepository.deleteToken(userId); // 토큰 db에서 제거
  ...
  public int insertRefreshToken(Map<String, Object> map) {
    ...
    int result = memberRepository.checkRefreshToken(map);

    if (result > 0) {
      int result2 = memberRepository.updateRefreshToken(map);
      if (result2 == 0) {
        throw new CustomException("리프레시 토큰 업데이트에 실패했습니다.", 500);
      }
      return result2;
    } else {
      int insertResult = memberRepository.insertRefreshToken(map);
      if (insertResult == 0) {
  ...
  public String smsCode(String phone) {
    ...
    MemberDto result = memberRepository.findById(phone);
  ...
  public List<MemberDto> recommendList(MemberDto m) {
    ...
    List<MemberDto> list = memberRepository.recommendList(map);
  ...
  public int signUp(MemberDto m) {
    ...
    int memberDto = memberRepository.signUp(m);
  ...
  public MemberDto insertInfo(MemberDto m, MultipartFile uploadFile) {
    ...
    int result = memberRepository.insertInfo(m);
    ...
    return memberRepository.login(mem)
            .orElseThrow(() -> new CustomException("회원정보가 없습니다.", 400));
  }
  ...
  public int recommendDelete(Map<String, String> map) {
    ...
    int result = memberRepository.recommendDelete(map);
    if (result == 0) {
  ...
  public int likeMember(Map<String, String> map) {
    ...
    LikeDto result = memberRepository.likeMemberCheck(map);

    if (result != null) {
      throw new CustomException("이미 좋아요를 누른상태입니다.", 401);
    } else {
      return memberRepository.likeMember(map);
    }
  ...
  public boolean duplicateCheck(String nickName) {
    ...
    try {
      int result = memberRepository.duplicateCheck(nickName);
      return result <= 0;
    } catch (Exception e) {
  ...
  public int interestMem(Map<String, String> map) {
    ...
    int check = memberRepository.interestMemCheck(map); // 이미 등록한 관심회원인지 확인
    if (check > 0) {
      throw new CustomException("이미 관심이 등록된 회원입니다.", 409);
    }


    int result = memberRepository.interestMem(map);
    if (result == 0) {
  ...
  public MemberDto detailInfo(String memId) {
    ...
    MemberDto m = memberRepository.detailInfo(memId);
    ...
    LikeDto ld = memberRepository.likeMemberCheck(map);
